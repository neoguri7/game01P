...
struct FLogger {
    ...
    static void Initialize(entt::registry& reg, bool enableFileLog = true) {
        ...
        if (enableFileLog) {
            auto file_sink = std::make_shared<spdlog::sinks::basic_file_sink_mt>("game01p.log", true);
...
// Convenience macros (use inside systems or game code that has access to registry)
...
namespace game::details {
    ...
    inline spdlog::logger* GetLoggerPtr() {
        // This relies on caller ensuring FLogger is emplaced in the registry.
        // In single-threaded game loop we can store a thread-local pointer after init.
        if (!g_currentLogger) {
            // fallback if globals needed (should not happen if ctx used)
...
inline void SetCurrentGameLogger(std::shared_ptr<spdlog::logger> ptr) {
    ::game::details::g_currentLogger = ptr;
}
